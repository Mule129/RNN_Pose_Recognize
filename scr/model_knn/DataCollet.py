...
class DataCollet():
    ...
    def cam_show(self, pose_list, fps_cnt):
        ...
        while True:
            ...
            if cnt >= len(pose_list):
                np.save(r"2022_AI_PJ\scr\model_knn\save_data\train_x.npy", collat_data)
                np.save(r"2022_AI_PJ\scr\model_knn\save_data\train_y.npy", collat_lable)
                break
            ...
            if loop >= fps_cnt:
                ...
                try:
                    cv2.putText(img, f"Next_pose : {pose_list[cnt]}", (250, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 4, cv2.LINE_AA)
                    cv2.putText(img, f"Next_pose : {pose_list[cnt]}", (250, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
                except IndexError:
                    cv2.putText(img, f"Next_pose : NULL", (250, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 4, cv2.LINE_AA)
